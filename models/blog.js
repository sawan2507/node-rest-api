...
const blog = {
  ...
  async update(params, id) {
    const connection = await db();
    const [rows] = await connection.execute('UPDATE `blogs` set `title` = ?, `content` = ?, `category_id` = ?, `image` = ? WHERE `id` = ?', [params.title, params.description, params.category_id, params.image, id]);
    return rows[0];
  },
