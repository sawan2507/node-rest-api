...
router.put('/update/:id', async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'Product updated success', 'status' : true, 'data': [] });
  } catch (error) {
    console.log(error)
    res.status(400).json({ message: 'Error Product updating', status:false, data: error });
  }
});
...
router.get('/list', async (req, res) => {
  try {
    ...
    const response = await Product.list(req.query);
    res.status(200).json({ message: 'Product list success', 'status' : true, 'data': response });
  } catch (error) {
    console.log(error)
    res.status(400).json({ message: 'Error listing products', status:false, data: {} });
  }
});
